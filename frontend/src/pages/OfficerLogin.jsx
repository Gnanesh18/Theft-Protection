...
import { ShieldCheck, Key, Mail, ArrowRight, AlertCircle, ArrowLeft, Eye, EyeOff } from 'lucide-react';
...
const OfficerLogin = () => {
  ...
  // If user is already logged in with officer or admin role, redirect. Otherwise, log out.
  useEffect(() => {
    if (user) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      if (loggedUser.role !== 'officer' && loggedUser.role !== 'admin') {
        logout();
        setSubmitting(false);
        setError('Access Denied: This portal is reserved for Officer accounts. Please use the correct login page.');
      } else {
        setSubmitting(false);
        navigate('/officer');
      }
    ...
    <div className="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative grid-bg min-h-[calc(100vh-4rem)]">
      ...
      >
        <div className="text-center relative">
          ...
          <h2 className="mt-3 text-2xl font-bold font-sans text-white">Officer Command Portal</h2>
          <p className="mt-1 text-xs text-slate-light">Law Enforcement Case Management System</p>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-2.5 text-sm font-semibold rounded-lg transition duration-200 flex items-center justify-center space-x-1 bg-blue-500 hover:bg-blue-600 text-white shadow-md"
          >
        ...
        <p className="text-center text-xs text-slate-light">
          No officer account registered yet?{' '}
          <Link to="/register/officer" className="text-white hover:underline font-semibold font-sans">
            Request Accreditation & Sign up
          </Link>
        </p>
      </motion.div>
    </div>
